...
public class CustomGlobal extends AbstractCustomGlobal {
	...
	public void treeButton() {
		/*
		 * int stages = Integer.parseInt(sinalgo.tools.Tools.
		 * showQueryDialog("Number of stages:")); if(stages <= 2) {
		 * sinalgo.tools.Tools.
		 * showMessageDialog("The number of stages needs to be at least" +
		 * "3.\nCreation of tree aborted."); return; }
		 * 
		 * int degree = Integer.parseInt(sinalgo.tools.Tools.
		 * showQueryDialog("Degree of the sink:")); if(degree <= 0) {
		 * sinalgo.tools.Tools.
		 * showMessageDialog("The degree of the sink needs to be at least" +
		 * "1.\nCreation of tree aborted."); return; } buildTree(degree,
		 * stages);
		 */
		buildTree(5, 6);
	}
	...
	private void buildGrid(int length, int width) {
		...
		// nodes
		sinalgo.tools.Tools.removeAllNodes();
		APDNode[][] theNodes = new APDNode[length][width];
	...
	private void buildStar(int degree, int numStages) {
		...
		// nodes
		APDNode[][] theNodes = new APDNode[degree][numStages];
		APDNode center;
		...
		for (int i = 1; i <= numStages; i++) {
			...
			for (int j = 0; j < degree; j++) {
				...
				if (i == numStages && j == 0)
					node = new InitNode();
				else if (i == numStages && j == degree / 2)
					node = new EndNode();
				else
					node = new APDNode();
				node.setPosition(posX, posY, 0);
	...
	private void buildTree(int degree, int numStages) {
		sinalgo.tools.Tools.removeAllNodes();
		ArrayList<APDNode> nodes = new ArrayList<APDNode>();
		...
		for (int i = 1; i < numStages; i++) {
			...
			posX = initPosX;
			for (int j = 0; j < degree; j++) {
				if (i == numStages - 1 && j == 0)
					node = new InitNode();
				else if (i == numStages - 1 && j == degree - 1)
					node = new EndNode();
				else
					node = new APDNode();
				node.setPosition(posX, posY, 0);
				node.finishInitializationWithDefaultModels(true);
				if (i == 1)
					addEdge(nodes.get(0), node);
				else
					addEdge(nodes.get(nodes.size() - degree), node);
				nodes.add(node);
				posX += initPosX;
			}
		}
